    ...
    <div style={{ marginTop: '20px' }}>
      ...
      {loading ? (
        ...
        <List
          ...
          renderItem={(item) => (
            <List.Item>
              <Card title={`Submitted on ${new Date(item.created_at).toLocaleString()}`}>
                <p><strong>Question:</strong> {item.question}</p>
                <p><strong>Status:</strong> {item.is_answered ? 'Answered' : 'Pending'}</p>
                {item.response && (
                  <p><strong>Answer:</strong> {item.response}</p>
                )}
              </Card>
            </List.Item>
          )}
