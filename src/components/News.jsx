  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <Row gutter={[24, 24]} justify="center">
        {news.map((article, i) => (
          <Col xs={24} sm={12} lg={8} key={i} style={{ display: 'flex', justifyContent: 'center' }}>
            <Card
              hoverable
              className="news-card"
              style={{
            ...
            >
              <a href={article.url} target="_blank" rel="noreferrer" style={{ textDecoration: 'none', color: 'inherit' }}>
                <div className="news-image-container" style={{ display: 'flex', justifyContent: 'center', marginBottom: '10px' }}>
                  <img
                ...
                <Title className="news-title" level={4} style={{ textAlign: 'center', marginBottom: '10px' }}>
                  {article.title.length > 60 ? `${article.title.substring(0, 60)}...` : article.title}
                </Title>

                <p style={{ textAlign: 'justify', color: '#555', marginBottom: '10px' }}>
                ...
                <div className="provider-container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <div style={{ display: 'flex', alignItems: 'center' }}>
                    ...
                    <Text className="provider-name" style={{ marginLeft: '8px' }}>
                      {article.source.name || 'Unknown Source'}
                    </Text>
                  </div>
                  <Text style={{ fontSize: '12px', color: '#888' }}>{moment(article.publishedAt).fromNow()}</Text>
                </div>
              </a>
            </Card>
          </Col>
